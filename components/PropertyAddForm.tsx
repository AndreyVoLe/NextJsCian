...
const PropertyAddForm = () => {
  ...
  const handleChange = (
    ...
    } else if (name.startsWith('seller_info.')) {
      // Обработка изменения для полей seller_info
      const sellerField = name.split('.')[1]
      setFields(prevFields => ({
        ...prevFields,
        seller_info: { ...prevFields.seller_info, [sellerField]: value },
      }))
    } else {
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      // Преобразуем FileList в массив и сохраняем объекты File
      const selectedFiles = Array.from(files)

      // Обновляем состояние, добавляя файлы
      setFields(prevFields => ({
        ...prevFields,
        images: selectedFiles, // Сохраняем массив объектов File
      }))
    }
  }
  console.log(fields)
